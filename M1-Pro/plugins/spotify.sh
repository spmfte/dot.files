...
update_track() {
    ...
    if [[ -z $SPOTIFY_JSON ]]; then
        sketchybar --set $NAME background.color=0x80c995da label.drawing=no
        return
    fi
    ...
    if [ $PLAYER_STATE = "Playing" ]; then
        ...
        sketchybar --set $NAME label="${TRACK}  ${ARTIST}" label.drawing=yes background.color=0x88a6da95

    elif [ $PLAYER_STATE = "Paused" ]; then
        sketchybar --set $NAME background.color=0x88eed49f
    elif [ $PLAYER_STATE = "Stopped" ]; then
        sketchybar --set $NAME background.color=0x88eed49f label.drawing=no
    else
        sketchybar --set $NAME background.color=0x88eed49f
    fi
}
